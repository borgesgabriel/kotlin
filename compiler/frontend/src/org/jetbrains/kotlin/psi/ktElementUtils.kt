...
internal fun KtElement.deleteWithSemicolon(): Boolean {
    ...
    if (sibling == null || sibling.node.elementType != KtTokens.SEMICOLON) return false

    val lastSiblingToDelete = PsiTreeUtil.skipSiblingsForward(sibling, PsiWhiteSpace::class.java)?.prevSibling ?: sibling
    parent?.deleteChildRange(this, lastSiblingToDelete)
    return true
}
