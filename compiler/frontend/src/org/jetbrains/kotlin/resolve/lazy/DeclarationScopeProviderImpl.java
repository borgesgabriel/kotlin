    ...
    public LexicalScope getResolutionScopeForDeclaration(@NotNull PsiElement elementOfDeclaration) {
        ...
        if (parentDeclaration instanceof KtClassOrObject) {
            ...
            if (ktDeclaration instanceof KtObjectDeclaration) {
                if (((KtObjectDeclaration) ktDeclaration).isCompanion()) return parentClassDescriptor.getScopeForCompanionObjectHeaderResolution();
                else return parentClassDescriptor.getScopeForStaticMemberDeclarationResolution();
            }

            if (ktDeclaration instanceof KtClass && !((KtClass) ktDeclaration).isInner()) {
