...
abstract class SuppressionManager {
    ...
    private fun isSuppressed(diagnostic: Diagnostic): Boolean {
        ...
        return isSuppressedByAnnotated(getDiagnosticSuppressKey(diagnostic), diagnostic.severity, annotated, 0)
    }

    public fun isSuppressed(psiElement: PsiElement, suppressionKey: String, severity: Severity): Boolean {
        // If diagnostics are reported in a synthetic file generated by KtPsiFactory (dummy.kt),
        // there's no point to present such diagnostics to the user, because the user didn't write this code
        ...
        val annotated = KtStubbedPsiUtil.getPsiOrStubParent(psiElement, KtAnnotated::class.java, false) ?: return false

        return isSuppressedByAnnotated(suppressionKey, severity, annotated, 0)
    }
